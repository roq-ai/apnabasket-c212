...
function OrderListPage() {
  ...
  const { data, error, isLoading, mutate } = useSWR<OrderInterface[]>(
    ...
    () =>
      getOrders({
        relations: ['user_order_customer_service_representative_idTouser', 'user_order_store_manager_idTouser'],
      }),
  );
            ...
            <Table variant="simple">
              <Thead>
                <Tr>
                  ...
                  {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                    <Th>user_order_store_manager_idTouser</Th>
                  )}
              ...
              <Tbody>
                {data?.map((record) => (
                  <Tr cursor="pointer" onClick={() => handleView(record.id)} key={record.id}>
                    ...
                    {hasAccess('user', AccessOperationEnum.READ, AccessServiceEnum.PROJECT) && (
                      <Td>
                        <Link as={NextLink} href={`/users/view/${record.user_order_store_manager_idTouser?.id}`}>
                          {record.user_order_store_manager_idTouser?.email}
                        </Link>
                      </Td>
                    )}
