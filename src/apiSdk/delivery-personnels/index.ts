...
export const getDeliveryPersonnels = async (query?: DeliveryPersonnelGetQueryInterface) => {
  const response = await axios.get(`/api/delivery-personnels${query ? `?${queryString.stringify(query)}` : ''}`);
